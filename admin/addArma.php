...
<html lang="es">
  ...
  <body>
    ...
    <div class="container">
      ...
      $im_corte = utf8_decode($_POST["im-corte"]);
      $crit_corte = utf8_decode($_POST["crit-corte"]);
      $dan_corte = utf8_decode($_POST["dan-corte"]);

      $im_pen = utf8_decode($_POST["im-pen"]);
      $crit_pen = utf8_decode($_POST["crit-pen"]);
      $dan_pen = utf8_decode($_POST["dan-pen"]);

      $im_apla = utf8_decode($_POST["im-apla"]);
      $crit_apla = utf8_decode($_POST["crit-apla"]);
      $dan_apla = utf8_decode($_POST["dan-apla"]);

      if (!($ps = $con->prepare("INSERT INTO Arma (nom, descr, tipo, im_corte, crit_corte, dan_corte, im_pen, crit_pen, dan_pen, im_apla, crit_apla, dan_apla) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , ?)"))) {
        die("<div class=\"alert alert-danger\"><strong>Impreparable:</strong> " . $con->error . "</div>\n");
      }

      if (!($ps->bind_param("ssssssssssss", $nom, $descr, $tipo, $im_corte, $crit_corte, $dan_corte, $im_pen, $crit_pen, $dan_pen, $im_apla, $crit_apla, $dan_apla))) {
